/*! ------------------------------------------------
 * Project Name: CodeMonks - Digital Agency & Personal Portfolio HTML Template
 * Project Description: Elevate your digital presence with CodeMonks - dynamic and stylish HTML template designed for creative agencies and personal brands. With modern layouts, smooth interactions and a polished aesthetic, CodeMonks template helps showcase projects, services and expertise with clarity and impact.
 * Tags: CodeMonks, resume, portfolio, personal page, cv, template, one page, responsive, html5, css3, creative, clean, agency, studio
 * Version: 1.0.0
 * Build Date: July 2026
 * Last Update: July 2026
 * This product is available exclusively on Themeforest
 * Author: CodeMonks
 * Author URI: https://themeforest.net/user/CodeMonks
 * File name: app.js
 * ------------------------------------------------
...
let pinnedSectionTriggers = [],
  stackSectionTriggers = [],
  viewportResizeTimer

function clearPinnedSectionTriggers() {
  pinnedSectionTriggers.forEach((e) => e && e.kill && e.kill()),
    (pinnedSectionTriggers = [])
}

function relayoutMasonryGalleries() {
  try {
    $(".mxd-projects-masonry__gallery").length &&
      $(".mxd-projects-masonry__gallery").masonry("layout")
  } catch (e) { }
}

function clearStackSectionTriggers() {
  stackSectionTriggers.forEach((e) => e && e.kill && e.kill()),
    (stackSectionTriggers = [])
}
...
function initStackSections() {
  ...
  clearStackSectionTriggers(),
    document.querySelectorAll(".stack-wrapper").forEach((e) => {
      const t = e.querySelectorAll(".stack-item"),
        a = e.querySelector(".stack-offset"),
        r = gsap.timeline()
      let o
      ...
      if (!isDesktop) {
        t.forEach((e) => gsap.set(e, { clearProps: "all" }))
        return
      }
      ...
      function n() {
        r.clear(),
          ...
          t.forEach((e, t) => {
            t > 0 &&
              (gsap.set(e, { y: t * o }),
                r.to(e, { y: 0, duration: 0.5 * t, ease: "none" }, 0))
          })
      }

      n()
      const i = ScrollTrigger.create({
        trigger: e,
        start: "top top",
        pin: !0,
        end: () => `+=${t.length * o + (a ? a.offsetHeight : 0)}`,
        scrub: !0,
        animation: r,
...
function initPinnedSections() {
  if ((clearPinnedSectionTriggers(), window.innerWidth < 1200)) return

  $(".mxd-pinned").each(function () {
    let e = $(this),
      t = e.find(".mxd-pinned__text-item"),
      a = e.find(".mxd-pinned__img-item"),
      r = -1

    if (!t.length || !a.length) return

    // These inline reveal styles can keep pinned content hidden.
    gsap.set(
      t.find(
        ".anim-uni-in-up, .anim-uni-scale-in-right, .anim-uni-scale-in-left, .anim-uni-scale-in"
      ),
      {
    ...
    function o(e) {
      if (e < 0 || e >= t.length || e === r) return
      r = e

      // Force deterministic desktop state even if other animations touch classes/styles.
      gsap.set(a, { autoAlpha: 0, zIndex: 1 })
      gsap.set(t, { opacity: 0.2 })
      ...
      gsap.set(a.eq(e), { autoAlpha: 1, zIndex: 2 })
      gsap.set(t.eq(e), { opacity: 1 })

      // Prevent stale inline animation styles from hiding active content.
      const n = t.eq(e).find(
        ".anim-uni-in-up, .anim-uni-scale-in-right, .anim-uni-scale-in-left, .anim-uni-scale-in"
      )
      n.length &&
    ...
    const n = ScrollTrigger.create({
      trigger: e[0],
      start: "top 85%",
      end: "bottom 15%",
      invalidateOnRefresh: !0,
      onEnter: () => o(0),
      onEnterBack: () => o(t.length - 1),
      onLeave: () => o(t.length - 1),
      onLeaveBack: () => o(0),
    })

    pinnedSectionTriggers.push(n)
    ...
    const i = () => {
      ...
      if (a.bottom <= e) return void o(t.length - 1)
      if (a.top >= e) return void o(0)

      let n = 0,
        i = Number.POSITIVE_INFINITY
      t.each(function (t) {
        const a = Math.abs(this.getBoundingClientRect().top - e)
        a < i && ((i = a), (n = t))
      }),
        o(n)
    }
...
// Helper function for testing (remove in production)
// function addTestButtons($section, $textItems, $imgItems, activateFunction) {
//   const testHTML = `
//     <div style="position: fixed; top: 10px; right: 10px; background: white; padding: 10px; border: 2px solid red; z-index: 9999;">
//       <strong>Debug Pinned Section:</strong><br>
//       ${Array.from({length: $textItems.length}, (_, i) => 
//         `<button onclick="window.activateTestItem(${i})" style="margin: 2px;">Item ${i}</button>`
//       ).join('')}
//     </div>
//   `;

//   $('body').append(testHTML);
//   window.activateTestItem = activateFunction;
// }
  ...
  if (e) {
    ...
    e.forEach((e) => {
      ; (t.el = e.querySelector(".marquee__toleft")),
        (t.width = t.el.offsetWidth),
        (t.el.innerHTML += t.el.innerHTML)
      let a = gsap.timeline().add(marquee(t.el, e.closest(".mxd-hero-01__marquee") ? 45 : 30, "-=50%"), 0),
        r = gsap.to(a, { duration: 1.5, timeScale: 1, paused: !0 }),
        o = gsap.utils.clamp(1, 6)
      ScrollTrigger.create({
        start: 0,
