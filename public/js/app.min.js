...
new Ukiyo(images, { scale: 1.5, speed: 1.5, externalRAF: !1 }),
  ...
  // Enhanced pinned section with better error handling
...
function initPinnedSections() {
  $(".mxd-pinned").each(function (sectionIndex) {
    let $pinnedSection = $(this);
    let $textItems = $pinnedSection.find(".mxd-pinned__text-item");
    let $imgItems = $pinnedSection.find(".mxd-pinned__img-item");

    console.log(`🎯 Initializing Pinned Section ${sectionIndex}:`, {
      textItems: $textItems.length,
      imgItems: $imgItems.length,
      isDesktop: window.innerWidth >= 1200
    });

    // Check if elements exist
    if ($textItems.length === 0 || $imgItems.length === 0) {
      console.error('❌ Missing text or image items in pinned section');
      return;
    }

    function activateItem(itemIndex) {
      if (itemIndex < 0 || itemIndex >= $textItems.length) return;

      console.log(`🔄 Activating item ${itemIndex}`);
      ...
      // Add active classes
      $textItems.eq(itemIndex).addClass("is-active");
      $imgItems.eq(itemIndex).addClass("is-active");

      // Force reflow for transitions
      void $textItems.eq(itemIndex)[0].offsetHeight;
      void $imgItems.eq(itemIndex)[0].offsetHeight;

      console.log(`✅ Activated: Text ${itemIndex} - ${$textItems.eq(itemIndex).hasClass('is-active')}, Image ${itemIndex} - ${$imgItems.eq(itemIndex).hasClass('is-active')}`);
    }

    // Initialize first item
    activateItem(0);

    // Create scroll triggers for each text item
    $textItems.each(function (itemIndex) {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          console.log(`📜 ScrollTrigger ENTER item ${itemIndex}`);
          activateItem(itemIndex);
        },
        onEnterBack: () => {
          console.log(`📜 ScrollTrigger ENTER BACK item ${itemIndex}`);
          activateItem(itemIndex);
        },
        onToggle: (self) => {
          if (self.isActive) {
            console.log(`📜 ScrollTrigger TOGGLE ACTIVE item ${itemIndex}`);
            activateItem(itemIndex);
          }
        }
      });
...
// Add this at the end of your script, after imgLoad.on("done")
function fixMobileInlineStyles() {
  if (window.innerWidth < 1200) {
    // Remove inline styles that break mobile
    document.querySelectorAll('.mxd-project-item__media > div[style]').forEach(el => {
      el.style.height = '';
      el.style.width = '';
    });

    document.querySelectorAll('.mxd-project-item__preview[style]').forEach(el => {
      el.style.height = '';
      el.style.width = '';
      el.style.transform = '';
    });
...
window.addEventListener('load', () => {
  setTimeout(fixMobileInlineStyles, 500);
});

// Call after scroll animations init
imgLoad.on("done", (e) => {
  hideLoader();
  pageAppearance();
  setTimeout(fixMobileInlineStyles, 1000);
});
